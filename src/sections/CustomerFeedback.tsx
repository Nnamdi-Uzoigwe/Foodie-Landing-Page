  ...
  return (
    <div className="my-20 md:my-40 grid grid-cols-1 lg:grid-cols-2 items-center gap-20 lg:gap-40">
      ...
      <div className="flex flex-col">
        <p className="text-center xl:text-left text-md text-gray-600">{testimonials[currentIndex].message}</p>

        <div className="flex items-center justify-between">
        <div className="mt-4 font-semibold">
          <img src={testimonials[currentIndex].imgSrc} alt="testimonial image" className="w-[50px] lg:w-[70px]" />
          <p className="text-[#e03328]">{testimonials[currentIndex].name}</p>
          ...
          {testimonials.map((_, index) => (
            <button
              ...
              className={`cursor-pointer w-4 h-4 rounded-full transition-colors duration-300 ${
                currentIndex === index ? "bg-[#e03328]" : "bg-transparent border-2 border-[#e03328]"
              }`}
